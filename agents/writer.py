...
from tools.common import get_anthropic, load_voice_guide, get_model
...
def _thread_caption_system(niche_id: str) -> str:
    """Build system prompt for thread captions, niche-aware."""
    niche = get_niche(niche_id)
    return f"""You're {niche['handle']}. {niche['description']}.

Rules:
- Describe what's IN this specific image — what detail, what material
- Add one fact the viewer can't see: dimension, cost, age, maker, technique
- Max 280 characters
    ...
    for i, img_path in enumerate(image_paths[1:], start=2):
        try:
            ...
            response = client.messages.create(
                model=VISION_MODEL,
                max_tokens=300,
                system=_thread_caption_system(niche_id),
                messages=[{
                    "role": "user",
                    "content": [
                        {
